...
var arcCreator = d3.arc().innerRadius(20).outerRadius(HEIGHT / 2 - 10); // pour que tout le camembert soit visible
...
pie.selectAll('path').data(pieData).enter().append('path').attr('d', arcCreator).attr('fill', color); // un texte pour chaque tranche

pie.selectAll('text').data(pieData).enter().append('text') // .centroid permet de trouver le centre de la tranche
.attr('transform', function (d) {
  return "translate(".concat(arcCreator.centroid(d), ")");
}).attr('text-anchor', 'middle').text(function (d) {
  return d.data.value;
}); // la légende

var legend = svg.append('g').attr('transform', "translate(".concat(HEIGHT - 10, ")"));
var RECT_WIDTH = 20; // les carrés de couleur

legend.selectAll('rect').data(pieData).enter().append('rect').attr('y', function (d, i) {
  return i * RECT_WIDTH;
}).attr('width', RECT_WIDTH).attr('height', RECT_WIDTH).attr('fill', color); // les noms de fruits

legend.selectAll('text').data(pieData).enter().append('text').attr('x', RECT_WIDTH * 1.5).attr('y', function (d, i) {
  return i * RECT_WIDTH + RECT_WIDTH * 0.75;
}).attr('width', RECT_WIDTH).attr('height', RECT_WIDTH).text(function (d) {
  return d.data.name;
});
...
//# sourceMappingURL=/graphique-d3.e31bb0bc.js.map